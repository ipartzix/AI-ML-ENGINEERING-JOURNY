# AI-ML-ENGINEERING-JOURNY
A structured end-to-end AI/ML engineering journey covering mathematics, machine learning, deep learning, large language models, MLOps, and production-grade projects. Built with a strong focus on fundamentals, implementation, and real-world systems.




# AI/ML Engineering Notes

Living notes for my AI/ML engineering journey — math, ML, DL, LLMs, and systems.

<p align="center">
  <a href="https://1drv.ms/o/c/1ffa72c4dd11fc4c/IgANhHTEUObLQblp3iYi27smAYPHXRHdGk45JmBt0m2NwXI?e=pOMyff" target="_blank">
    <img src="https://img.shields.io/badge/View-OneNote%20Notes-blue?style=for-the-badge&logo=microsoftonenote" />
  </a>
</p>

## Status
- Notes are actively maintained
- Content is evolving
- Repository acts as an index + roadmap
